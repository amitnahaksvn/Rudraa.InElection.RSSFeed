...
export function useNewsFeed(
  ...
  return useInfiniteQuery({
    // sortBy/sortDirection are part of the key (not just params) so switching either starts the
    // infinite scroll over from page 1 instead of re-sorting whatever pages happened to already
    // be loaded.
    queryKey: ['newsFeed', sourceType, country, sortBy, sortDirection],
    queryFn: ({ pageParam }) =>
      fetchNewsFeed({ sourceType, country: country ?? undefined, skip: pageParam, count: PAGE_SIZE, sortBy, sortDirection }),
    initialPageParam: 0,
